...
var colors = [ Ted", "blue", "green*】；//胡建一个包含3个字许亊的数组 var names = [] ；	//创建■—个空数组
var values = [ 1,2,J；	// 不要这样！这样会创建一个&含2或3項的数組
var options = I,,,,, 1;	//不要这样！这样会创建一个包含5或6項的数紐
ArrayTypeExampleOl. htm
以上代码的第一行创建了一个包含3个字符串的数组。第二行使用一对空方括兮创建了一个空数组。 第三行展示/■在数组字面量的最后一项添加逗号的结果：在IE中，values会成为一个包含3个项且每 项的值分别为1、2和undefined的数组；在其他浏览器中，values会成为-个包含2项且值分别为 1和2的数组。原W是IE8及之前版本中的ECMAScript实现在数组字面量方面存在bug。由于这个bug

5.2 Array 类型 87
导致的另一种情况如最后一行代码所示，该行代码可能会创建包含5项的数组（在IE9+、Firef〇x、Opera、 Safari和Chrome中）,也可能会创建包含6项的数组（在IE8及吏早版本中）。在像这种省略值的情况下， 每一项都将获得undefined值；这个结果与调用Array构造函数时传递项数在逻辑上是相同的。但是 由于IE的实现与其他浏览器不一致，因此我们强烈建议不要使用这种语法。
f
	与对象一样，在使用数组字面量表示法时，也不会调用Array构造函数（Firefox3
及更早版本除外）。
在读取和设数组的值时，要使用方栝号并提供相应值的基于0的数字索引，如下所示:
var colors = ["red", "blue"/ "green*];	// 定义一个字符爭数纽
alert (colors [0]);	// 示第一項
colorsf21 = "black"；	// 修改第三項
...
var colors = t •'red", "blue", "grocn" ] ；	// 创建一个包含 3 个字符申的数组
var names = [];	// 釗建一个空数组
alert(colors.length)；	//3
alert(names.length)；	//〇
数组的length属性很有特点一它不是只读的。因此，通过设置这个属性，可以从数组的末尾移 除项或向数组中添加新项。请看下面的例子：
var colors = I "red", -blue", "green"]； // 创建一个&含 3 个字符串的数组 colors昹ength _ 2;
...
var colors = [ "red-, "blue", "green" ] ；	// 创建—^个包含 3 个字符亊的教组
colora.length s 4;
alert(colors【3 】）；	//undefined
A rrgy TypeExample04.htm
在此，M然colors数组包含3个项，但把它的length属性设置成了 4。这个数组不存在位置3, 所以访问这个位置的值就得到了特殊值undefined。
利用length属性也可以方便地在数组末M添加新项，如下所示：

var colors = ["red", "blue1*, "green") ；	// 创建一个包含 3 个字符 _ 的数组
colors [colors. length] = "blacJc";	// (在位篁3>添加一种顏色
colors [colors.length] * "brown";	//(在位里 4)再添扣一种相色
A rrayTypeExample05. htm
...
var colors = ["red", "blue", "green");	// 创建一个包含 3 个字符♦的数组
colors[99] = "black"；	//(在位里 99)添加一种麻色
alert(colors.length); // 100
ArrqyTypeExample06.htm
...
aX«rt(colors«toString());	// red,blue,gre«n
alert.(colors.valueOf 〇 );	// red,blue,green
alert(colore)；	// red,blue # green
ArrayTypeExampleO 7. him
...
var
person2 = {
toLocaleString ： function 〇
return "Grigorios*；
{

toString ： function() { return "Greg-；
var people = [personl, person2]； alert(people); alert(people.toString())； alert (people. toLocaleString ⑴；
//Nicholas,Greg //Nicholas,Greg //Nikolaos,Grigorios
Array TypeExample08. htm
...
alert(colors.join("II"));	//red|Igreen丨丨 blue
A rray TypeJoinExample01.htm
...
var colors = ("red", "blue"1;
colors.push("brown")；
colors[3】="black";
alert(colors.length)；	// A
var item = colors.popO ;
alert(item);	//"black"
ArrayTypeExampieW.htm
//添加另一項 //添加一項
//取得最后一唷
...
用队列一样使用数组。
var colors = new Array () ；	//创建一个数
var count = colors .push ("red*, "green");	//推入两嘴
alert(count)；	//2
count = colors.push("black")
alert(count);	//3
...
5.2 Array 类型 93
function compare(valuel, value2) { if (valuel < value2) { return -1；
} else if (valuel > value2) { return 1;
} else {
return 0；
>
>
ArrayTypeExampleI5.htm
这个比较函数可以适用于大多数数据类型，只要将其作为参数传递给sort ()方法即可，如下面这 个例子所示。
var values = t〇, 1, 5, 10, 15)； values.sort(compare);
alert(values)；	//0,1,5,10,IS
...
var colors = [*red*, "green", •blue", "yellow", "purple"]; var colors2 = colors.sliced); var colors3 = colors.slice(l,4);
alert(colors2)；	//green,blue,yellow,purple
alert(colors3);	//green # blue,yellow
ArrayTypeSliceExampleOI.htm
...
ArrayFilterExampleOLhtm
这里，通过调用filters)方法创建并返丨租了包含3、4、5、4、3的数组，因为传入的函数对它们 每一项都返In] true。这个方法对査询符合某共条件的所有数组项非常有用。
map(>也返回一个数组，而这个数组的每一项都是在原始数组中的对应项h运行传人函数的结果。 例如，可以给数组中的每一项乘以2,然后返冋这些乘积组成的数组，如下所示。 var numbers = [1,2,3,4,5,4,3,2,1];
var mapResult = numbers.map(function(item, index, array){ return item * 2；
))；
alert(mapResult);	//[2,4,6,8,10,8,6,4,2]
ArrayMapExampleOl.htm
以上代码返回的数组中包含给每个数乘以2之后的结果。这个方法适合创建包含的项与另一个数组 一一对应的数组。
最后一个方法是f〇rEach(),它只是对数组中的每一项运行传入的函数。这个方法没有返回值， 本质上与使用for循环迭代数组一样。来看一个例子。 var numbers = [1,2,3,4,5,4,3,.2,1];
...
使用reduce (K方法uJ■以执行求数组屮所有值之和的操作，比如： var values = (1,2,3,4,51;
...
alert: ihtmlEscape ("<p class=\Mgreeting\->Hel lo world !</p>" >); //&lt;p class=&quot?greeting&quot;&gt；Hello world!&lc；/p&gt;
...
www.wrox.com/illegal value.htm ),而 encode- URIComponent(>主要用于对URI中的某一段（例如前面URI中的illegal value.htm)进行编码。 它们的主要区别在于，encodeURIU不会对本身属于URI的特殊字符进行编码，例如冒号、正斜杠、 问号和井字号；而encodeURIComponent (>则会对它发现的任何非标准字符进行编码。来看下面的例子。 var uri = "http：//vavw.wrox.com/illegal value.htm#start";
//"http：//

www.wrox.com/illegal%20value.htm#start" alert(encodeURI(uri));
...
var max = Math.max(3, 54, 32, 16) ? alerc(max)；	//54
var min = Math.min(3# b4• 32, 16); alert(min);	//3
MathObjectM\nMaxExampleOl.htm
...
下面是使用这些方法的示例：
MathObjectRoundingExampleQl.htm
...
6 6 6 6 6 5 5 5 5 2 2 2 2 2 2 2 2 2 /// /// /// /// //////
...
MathObjectRandomExampleO I. htm
总共有】0个可能的值（1到〗0),而第-个可能的值是1。而如果想要选择一个介于2到10之间的 值，就应该将h面的代码改成这样：
var nxnn s Math.floor(Kach.random() * 9 + 2);
MathObjectRandomExample02. htm
从2数到10要数9个数，因此可能值的总数就是9, ifij第一个可能的值就是2。多数情况下，其实 都可以通过一个阐数来计算可能值的总数和第一个吋能的值，例如：
function selectFroir.dowerValue, upperValue) { var choices = upperValue - lowerValue + l.; return Math, f lo〇r{Math .random() * choices -*• lowerValue)；
var num ^ selectFroir.(2, 10}；
alert (num) ;	//介于2和10之间（包括2和10)的一个数值
MathObjectRandomExampleO^. htm
