...
&emsp;&emsp;□基本类型值在内存中占据固定大小的空间，因此被保存在栈内存中;  
&emsp;&emsp;□从一个变最向另一个变量复制基本类型的值，会创建这个值的一个副本;  
&emsp;&emsp;□引用类型的值是对象，保存在堆内存中;  
&emsp;&emsp;□包含引用类型值的变童实际上包含的并不是对象本身，而是一个指向该对象的指针;    
&emsp;&emsp;□从一个变量向另一个变量复制引用类型的值，复制的其实是指针，W此两个变量最终都指向同 一个对象;  
&emsp;&emsp;□确定-个值是哪种基木类铟可以使用typeof操作符，而确定一个值是哪种引用类型可以使用 instanceof 操作符〇  
所有变ft (包括基本类型和引用类型）都存在于一个执行环境（也称为作用域）当中，这个执 行环境决定了变童的生命周期，以及哪一部分代码可以访问其中的变量。以下是关于执行环境的几 点总结：    
&emsp;&emsp;□执行环境有全局执行环境（也称为全局环境）和函数执行环境之分;  
&emsp;&emsp;□每次进人一个新执行环境，都会创建一个用T搜索变量和函数的作用域链;  
&emsp;&emsp;口函数的局部环境不仅有权访问函数作用域中的变童，而且有权访问其包含（父）环境，乃至全 局环境;  
&emsp;&emsp;□全M环境只能访问在全W环境中定义的变置和函数，而不能直接访问局部环境中的任何数据;  
